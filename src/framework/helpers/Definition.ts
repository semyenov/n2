...
type HttpRouteOptions<Rpcs extends Rpc.Any, R> = {
  readonly handlers: Layer.Layer<Rpc.ToHandler<Rpcs>, never, R>
}
...
> {
  ...
  readonly toHttpRoute: {
    <Rpcs extends Rpc.Any>(
      group: RpcGroup.RpcGroup<Rpcs>,
      path: `/${string}`,
      options: HttpRouteOptions<Rpcs, R>
    ): Layer.Layer<never, never, R>
    <Rpcs extends Rpc.Any, Result, MappedErr>(
      group: RpcGroup.RpcGroup<Rpcs>,
      path: `/${string}`,
      options: AdapterOptions<"rpc", State, Command, Event, Result, MappedErr>
    ): Layer.Layer<never, never, R>
...
<State, Err, R>(options: {
  ...
  function toHttpRoute<Rpcs extends Rpc.Any>(
    group: RpcGroup.RpcGroup<Rpcs>,
    path: `/${string}`,
    routeOptions: HttpRouteOptions<Rpcs, R> | AdapterOptions<"rpc", State, Command, Event, unknown, unknown>
  ) {
    const handlers =
      "handlers" in routeOptions
        ? routeOptions.handlers
        : toRpcHandlers(group, routeOptions)

    return makeRoute(group, path, handlers)
  }
